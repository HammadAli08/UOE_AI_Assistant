...
def extract_semester(text: str) -> int:
    """Extract semester number (0 if not found). No hard range limit."""
    patterns = [
        r'Semester\s*[-–—]?\s*([IVX]+)',
    ...
    if re.search(r'course code[:\s]', text_lower) or \
       (re.search(r'course title[:\s]', text_lower) and
        re.search(r'(course outline|course content|specific objective|clo|course description|'
                  r'learning outcomes|suggested readings|recommended books)', text_lower)):
        return "course_detail"

    # ── Semester subject list: has "semester" + tabular course listings ──
    if re.search(r'semester\s*[-–]?\s*[ivx\d]', text_lower) and \
       re.search(r'(sr\.?\s*no|course code|course title|cr\.?\s*hrs?|credit)', text_lower):
